...
const ProductPage = () => {
  ...
  const [currentImages, setCurrentImages] = useState(data[0].variants[colorIndex].images)

  useEffect(() => {
    setSize(data[0].variants[colorIndex].sizes[0])
    setCurrentImages(data[0].variants[colorIndex].images)
  }, [colorIndex])

  const discountedPrice = data[0].price - data[0].price * (data[0].discountPercent / 100)
      ...
      <div className="flex my-10">
        <div className="flex flex-col w-1/2">
          ...
          <Accordion type="single" collapsible className="w-full mt-10">
            ...
            <AccordionItem value="item-3">
              ...
              <AccordionContent>
                <ReactMarkdown>{data[0].care_guide.replace(/(\d+\.)\s*/g, '\n\n$1')}</ReactMarkdown>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        </div>
        <div className="flex flex-col w-1/2 px-14">
          ...
          <div className="flex py-5">
            {data[0].discountPercent > 0 ? (
              <>
                <div className="pr-6 text-2xl font-semibold text-red-600">${discountedPrice}</div>
                <div className="pr-6 line-through text-base text-muted-foreground">
                  ${data[0].price}
                </div>
              </>
            ) : (
              <div className="pr-8 text-2xl font-semibold text-black">${data[0].price}</div>
            )}
          </div>
          <div className="text-sm text-gray-800">
            <b className="text-base text-black">{data[0].name} highlights: </b>
            <span className="line-clamp-5">{data[0].highlights}</span>
          </div>
          <div className="flex flex-col py-5">
            ...
            <div className="w-full">
              ...
              <RadioGroupCustom defaultValue={size} onValueChange={setSize}>
                {SIZE_RANGES[data[0].male as keyof typeof SIZE_RANGES].map(
                  (availableSize: number) => {
                    const isAvailable = data[0].variants[colorIndex].sizes.includes(
